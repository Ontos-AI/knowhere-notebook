...
- **Build:** `pnpm build`
- **Docker image:** `docker build -t knowhere-notebook:dev .` then `docker run -d --name knowhere-notebook -p 3001:3000 --add-host localhost.localstack.cloud:host-gateway --env-file .env.docker knowhere-notebook:dev` (standalone, non-root, port 3000). The `--add-host` flag is required for self-hosted Knowhere with LocalStack S3 so the container can resolve `localhost.localstack.cloud` to the host gateway (used for fetching table/image chunk assets server-side).
