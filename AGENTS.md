...
- **Encrypted API keys (Phase 3):** workspaces manage Knowhere keys via the switcher's "API keys…" dialog (`src/components/workspace-api-keys-dialog.tsx`) backed by `src/app/api/workspaces/[workspaceId]/api-keys` routes. Keys are AES-256-GCM encrypted at rest (`knowhere_api_keys` table; `KNOWHERE_KEY_ENCRYPTION_KEY` env — 32-byte base64) and never shown after save. `workspaces.active_knowhere_api_key_id` picks the active key.
